...
class DecisionRouter {
    ...
    assessUrgency(query) {
        ...
        if (criticalIndicators.some(i => lowerQuery.includes(i)))
            return 'critical';
        if (highIndicators.some(i => lowerQuery.includes(i)))
            return 'high';
        if (lowIndicators.some(i => lowerQuery.includes(i)))
            return 'low';
        return 'normal';
    }
    /**
    ...
    assessStakes(query) {
        ...
        if (highIndicators.some(i => lowerQuery.includes(i)))
            return 'high';
        if (mediumIndicators.some(i => lowerQuery.includes(i)))
            return 'medium';
        if (lowIndicators.some(i => lowerQuery.includes(i)))
            return 'low';
        return 'minimal';
    ...
    determineTier(factors) {
        ...
        if (factors.stakes === 'critical' && factors.requiresApproval)
            return 'human';
        if (factors.legalCompliance && factors.stakes === 'critical')
            return 'human';
        if (factors.safetySensitive && factors.stakes === 'critical')
            return 'human';
        if (factors.requiresApproval && factors.stakes === 'high')
            return 'human';
        if (factors.emotionalContent && factors.stakes === 'high')
            return 'human';
        // Brain tier triggers
        if (factors.complexity === 'complex' || factors.complexity === 'extreme')
            return 'brain';
        if (factors.hasCode && factors.complexity !== 'trivial')
            return 'brain';
        if (factors.requiresCreativity && factors.novelty > 0.5)
            return 'brain';
        if (factors.requiresJudgment && factors.stakes === 'medium')
            return 'brain';
        if (factors.novelty > 0.7)
            return 'brain';
        // Bot tier for simple queries
        if (factors.complexity === 'trivial' || factors.complexity === 'simple') {
            if (factors.stakes === 'minimal' || factors.stakes === 'low')
                return 'bot';
        }
        if (!factors.requiresCreativity && !factors.requiresJudgment && factors.novelty < 0.3)
            return 'bot';
    ...
    calculateConfidence(factors) {
        ...
        if (factors.stakes === 'critical')
            confidence += 0.15;
        if (factors.complexity === 'trivial')
            confidence += 0.1;
        if (factors.novelty < 0.3)
            confidence += 0.1;
     ...
     */
    generateReasoning(factors, tier) {
        const reasons = [];
        reasons.push(`Complexity: ${factors.complexity}`);
        reasons.push(`Stakes: ${factors.stakes}`);
        if (factors.stakes === 'critical')
            reasons.push('Critical stakes detected');
        if (factors.requiresApproval)
            reasons.push('Requires approval');
        if (factors.legalCompliance)
            reasons.push('Legal compliance needed');
     ...
     */
    identifyEscalationTriggers(factors) {
        const triggers = [];
     ...
     */
    buildDecision(tier, factors, reason) {
        return {
...
//# sourceMappingURL=DecisionRouter.js.map