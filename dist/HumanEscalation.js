...
exports.HumanEscalation = void 0;
/**
...
class HumanEscalation {
     ...
     */
    async request(escalationRequest) {
     ...
     */
    updateOperatorAvailability(operatorId, available) {
        const operator = this.operators.get(operatorId);
        if (operator) {
    ...
    findAvailableOperator(request) {
        ...
        const scored = availableOperators.map(op => {
            ...
            if (request.metadata?.category) {
                const hasSpecialization = op.specializations.some(s => s.toLowerCase() === (request.metadata?.category).toLowerCase());
...
exports.default = HumanEscalation;
//# sourceMappingURL=HumanEscalation.js.map